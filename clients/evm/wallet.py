...
class WalletClient(BaseWeb3Client):
    def __init__(self, chain_config, private_key: str | None = None):
    ...
    async def wait_transaction(self, tx_hash: HexStr):
        receipt = await self.wait_for_transaction(tx_hash, timeout=30)
        return receipt
