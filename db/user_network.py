...
class UserNetworkSettings(Base, TimestampMixin):
    ...
    user = relationship("User", back_populates="network_settings")
    network = relationship("Network", back_populates="user_settings")

    __table_args__ = (
        UniqueConstraint('user_id', 'network_id', name='uq_user_network'),
        Index('idx_user_network_enabled', 'user_id', 'network_id', 'is_enabled'),
        CheckConstraint('buy_slippage >= 0.1 AND buy_slippage <= 1000', name='check_buy_slippage'),
        CheckConstraint('sell_slippage >= 0.1 AND sell_slippage <= 1000', name='check_sell_slippage'),
        CheckConstraint('buy_gas_delta >= 1.5 AND buy_gas_delta <= 1000000', name='check_buy_gas_delta'),
        CheckConstraint('sell_gas_delta >= 1.5 AND sell_gas_delta <= 1000000', name='check_sell_gas_delta'),
        CheckConstraint('approve_gas_delta >= 1.5 AND approve_gas_delta <= 1000000', name='check_approve_gas_delta'),
    ...
    def __repr__(self) -> str:
        return (
            f"<UserNetworkSettings(id={self.id}, user_id={self.user_id}, "
            f"network_id={self.network_id}, is_enabled={self.is_enabled})>"
        )
